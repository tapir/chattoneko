...
Two sources merged into one catalog (`tools.Merge(cfg, sources...)`): integrated tools first (they win name collisions), then MCP tools. The merge also layers the config's `tool_defaults` map (tool name → default enabled, edited in the settings UI) over the sources' own defaults: listed tools report that default, unlisted ones keep their source's own. The merged catalog is the only tool interface the engine and API consume, so `GET /api/config` reports the same `default_enabled` the engine uses and a settings save applies live (both the sources and the map are read per `Tools()` call).
...
| `POST /api/setup/mcp/tools` | accepts ONE MCP server's `{name, url, headers}` — unsaved settings-form values are the point — and returns its tools as `[{name, description}]` via `mcphub.Probe`. Saves nothing, connects nothing, leaves the live catalog untouched; 400 without a url, 502 when the server can't be dialed or won't list. |
