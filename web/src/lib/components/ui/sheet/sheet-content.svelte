...
<SheetPortal {...portalProps}>
	...
	<SheetPrimitive.Content
		...
		data-side={side}
		class={cn(
			// p-safe clears the Android status/nav bars in the edge-to-edge WebView.
			// Unlayered so it beats caller padding classes; env() is 0 on desktop.
	...
	>
		{@render children?.()}
		{#if showCloseButton}
			<!-- Absolute offsets measure from the padding-box edge, so this button
			     ignores p-safe and applies the inset itself; max() keeps the
			     0.75rem offset where there is no inset. -->
