...
- **Vite 8** (rolldown) — dev server proxies `/api` to `localhost:8080`; the production build splits the heavy markdown pipeline (`marked` + `temml` + `highlight.js` + `dompurify`) into its own chunk via the function form of `manualChunks`, and `chunkSizeWarningLimit` is raised to 600 kB because a single-view chat app has no route to split the entry along.
...
- **Markdown pipeline**: `marked` → `temml` (LaTeX) → `highlight.js` (code) → `DOMPurify` (sanitization), lazily loaded (see **Markdown**).
...
lib/
  ...
  markdown.js               façade: lazy loader + sync render signature
  markdown.core.js          pure string helpers (escapeHtml, normalizeMathDelimiters, stablePrefixLen)
  markdown.impl.js          the heavy pipeline (marked + Temml + highlight.js + DOMPurify) — own chunk
...
- **`send(content, attachments)`**: pushes an optimistic user bubble FIRST (local `pending-` id) so a slow link doesn't leave the message area blank across the round trips, then `ensureChat()` — creates the chat server-side (`POST /chats`) on the first send, carrying the composer's model, reasoning effort and per-chat tool overrides; the returned `created` flag lets the failure path roll the empty chat back (delete it) so it never lingers in the sidebar. Then uploads attachments and sends the message. The persisted user message replaces the optimistic bubble in place via `adoptUserMessage` (object swap, not id mutation — `MessageList` keys its `{#each}` on `m.id`); `dropPendingUser` removes it if the send fails or the response omits it. Finally it pushes the assistant placeholder optimistically, starts the live state, and `kick()`s the chat stream so the new generation is picked up immediately instead of on the idle poll.
...
- **`TitleStream`** (`/api/stream/titles`) applies final auto-generated titles to the sidebar entry and the open chat's header. All three rely on EventSource semantics; only ChatStream needs manual retry logic.

Event vocabulary handled by the store: `idle`, `generation_started`, `delta`, `reasoning_delta`, `tool_call_started`, `tool_call_delta`, `tool_call_done`, `tool_result`, `attachment_created`, `status`, `done`, `user_message` (our own send — it can beat the POST response, so it adopts the optimistic bubble — or one sent from another client), `chat_updated`, `settings_updated` (per-chat settings changed elsewhere — applied without refetch), `messages_reset` (history truncated elsewhere — refetch the chat), `config_changed` (MCP catalog rebuilt — refetch `/api/config`).

## Markdown

Three modules keep the heavy third-party stack off the critical path:

- `markdown.core.js` — pure string helpers with zero deps (`escapeHtml`, `normalizeMathDelimiters`, `stablePrefixLen`), bundled into the entry chunk. `stablePrefixLen` is on the streaming hot path.
- `markdown.impl.js` — the real pipeline (`marked` + Temml + highlight.js + DOMPurify, ~135 kB gzipped), reached ONLY through the dynamic `import()` in `markdown.js`. A static import anywhere in the reachable graph merges it back into the entry chunk.
- `markdown.js` — the façade every call site uses. `loadMarkdown()` is idempotent (one promise, one request), `onMarkdownReady(fn)` lets a component `$effect` re-run when the chunk lands, and `renderMarkdown(src)` stays synchronous: while the chunk is in flight it self-triggers the load and returns the source as an escaped, newline-preserving span, so a message is readable immediately and upgrades in place. Nothing renders blank.
...
- **MessageItem** streaming optimization: the typewriter grows content a few characters per animation frame, and only the STABLE PREFIX (complete lines) goes through the markdown pipeline — at most every 90ms (`PROMOTE_MS`); the incomplete tail renders as a plain-text span at zero parse cost. Terminal messages parse once. Tool-created files render as download chips only after `done` (they don't pop in mid-stream). Failed generations show `GenerationError`; stopped/failed states keep partial output visible.
...
- Temml's MathML stylesheet and the highlight.js token palettes are imported/inlined here.
