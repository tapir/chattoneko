...
- `jq` — runs a jq filter over a JSON value the model supplies, fed in on stdin, and returns what the filter produces as compact JSON. Exact arithmetic, genuine regular expressions (`test`/`match`/`capture`), and querying, reshaping, sorting, grouping or counting structured data in one filter instead of a whole program. Nothing outside that value is reachable — no files, no command-line arguments, no network, and the environment is empty so `$ENV` yields `{}`; capped by time and by output size.
