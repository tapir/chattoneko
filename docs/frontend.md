...
lib/
  ...
  markdown.core.js          pure string helpers (escapeHtml, normalizeMathDelimiters, stablePrefixLen, closeOpenInline)
...
- `markdown.core.js` — pure string helpers with zero deps (`escapeHtml`, `normalizeMathDelimiters`, `stablePrefixLen`, `closeOpenInline`), bundled into the entry chunk. `stablePrefixLen` and `closeOpenInline` are on the streaming hot path.
...
- **MessageItem** streaming optimization: the typewriter grows content a few characters per animation frame, and only the STABLE PREFIX (complete lines) goes through the markdown pipeline — at most every 90ms (`PROMOTE_MS`). The incomplete tail is a single unfinished line, so it IS parsed per frame: `closeOpenInline` appends the closers a mid-line cut left open (`` ` ``, `**`, `~~`, `](`) and the result renders through the pipeline with its wrapping `<p>` stripped, so bold/code/links appear immediately instead of as literal markers. It stays a literal escaped span when `closeOpenInline` returns null (fence line, unclosed `$$`/`\[` math) or when the prefix ended inside an open ``` fence (`tailCode`). Terminal messages parse once. Tool-created files render as download chips only after `done` (they don't pop in mid-stream). Failed generations show `GenerationError`; stopped/failed states keep partial output visible.
