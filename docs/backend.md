...
- SSE handler writes `event: message` + JSON `data` lines, flushes every event, and sends `: ping` comments every 20s to keep connections alive. Long-lived SSE paths are excluded from request logging.
