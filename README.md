...
- `speak` — reads text aloud and puts the recording in the conversation as an audio player, the same way `attach` puts a file there. Any message can also be read aloud straight from its action row, which streams the audio without storing it.
